...
class AuthCore {
    ...
    public function register($username, $email, $password, $confirm_password, $user_type = 'user') {
        ...
        $stmt = $this->conn->prepare("INSERT INTO users (username, email, password, user_type, created_at) VALUES (?, ?, ?, ?, NOW())");
        if (!$stmt) {
            error_log("Prepare failed: " . $this->conn->error);
            return ['success' => false, 'errors' => ['Błąd bazy danych. Spróbuj ponownie.']];
        }
    ...
    public function login($username, $password) {
        ...
        $stmt = $this->conn->prepare("SELECT id, username, email, password, user_type FROM users WHERE username = ? OR email = ?");
        if (!$stmt) {
            error_log("Prepare failed: " . $this->conn->error);
            return ['success' => false, 'errors' => ['Błąd bazy danych. Spróbuj ponownie.']];
        }
...
public function logout() {
    ...
    public function getConnection() {
        // Check if connection is still alive (PHP 8.4+ compatible)
        if ($this->conn && mysqli_stat($this->conn)) {
            return $this->conn;
        }
    ...
    public function createTables() {
        ...
        if ($this->conn->query($sql) && $this->conn->query($login_attempts_sql) && $this->conn->query($remember_tokens_sql)) {
            return true;
        } else {
            return false;
        }
    ...
    private function isBlocked($ip_address) {
        $stmt = $this->conn->prepare("SELECT COUNT(*) as attempts FROM login_attempts WHERE ip_address = ? AND attempt_time > DATE_SUB(NOW(), INTERVAL ? SECOND)");
    ...
    public function checkRememberCookie() {
        if (isset($_COOKIE['remember_token']) && !$this->isLoggedIn()) {
            ...
            $stmt = $this->conn->prepare("SELECT rt.user_id, u.username, u.email, u.user_type FROM remember_tokens rt JOIN users u ON rt.user_id = u.id WHERE rt.token = ? AND rt.expires_at > NOW()");
            if ($stmt) {
