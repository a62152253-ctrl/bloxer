<?php
require_once __DIR__ . '/../controllers/core/mainlogincore.php';
...
foreach ($testAccounts as $account) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($result['success']) {
            ...
            $updateStmt = $conn->prepare("UPDATE users SET bio = ?, avatar_url = ?, github_url = ?, twitter_url = ?, website = ? WHERE id = ?");
            
            // Parse social links
            $socialLinks = json_decode($account['social_links'] ?? '{}', true);
            $githubUrl = $socialLinks['github'] ?? null;
            $twitterUrl = $socialLinks['twitter'] ?? null;
            $website = $socialLinks['portfolio'] ?? null;
            ...
            if ($account['user_type'] === 'developer') {
                $walletStmt = $conn->prepare("INSERT INTO developer_wallet (user_id, balance, total_earned, total_withdrawn) VALUES (?, ?, ?, ?)");
                $totalEarnings = rand(100, 5000);
...
echo "<p><a href='../controllers/auth/login.php'>Przejdź do strony logowania</a></p>";
?>
