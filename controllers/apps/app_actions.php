...
switch ($action) {
    ...
    case 'toggle_save':
        ...
        $stmt->bind_param("ii", $user['id'], $app_id);
        if (!$stmt->execute()) {
            echo json_encode(['success' => false, 'error' => 'Database error during favorite toggle']);
            break;
        }
        ...
        if ($result->num_rows === 0) {
            ...
            if ($stmt) {
                $stmt->bind_param("ii", $user['id'], $app_id);
                $stmt->execute();
                echo json_encode(['success' => true, 'is_favorite' => 1, 'message' => 'Added to favorites']);
            } else {
                echo json_encode(['success' => false, 'error' => 'Database error during favorite creation']);
            }
        } else {
            ...
            if ($stmt) {
                ...
                $stmt->execute();
                echo json_encode(['success' => true, 'is_favorite' => $new_favorite, 'message' => $new_favorite ? 'Added to favorites' : 'Removed from favorites']);
            } else {
                echo json_encode(['success' => false, 'error' => 'Database error during favorite update']);
            }
        }
        break;
    ...
    case 'review':
        ...
        $stmt->bind_param("ii", $user['id'], $app_id);
        if (!$stmt->execute()) {
            echo json_encode(['success' => false, 'error' => 'Database error during review check']);
            break;
        }
        if ($stmt->get_result()->num_rows === 0) {
